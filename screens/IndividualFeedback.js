...
const IndividualFeedback = ({ navigation }) => {
  ...
  const fetchFeedbackData = async () => {
    try {
      const response = await fetch(
        "http://192.168.81.53:3002/admin/individual-feedback",
        {
          method: "GET",
          headers: {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerText}>Individual Feedback</Text>
      </View>
      ...
      <ScrollView style={styles.feedbackList}>
        {feedbacks.map((feedback) => (
          <View key={feedback.id} style={styles.feedbackItem}>
            ...
            <View style={{ flexDirection: "column", justifyContent: "center" }}>
              <Text style={styles.label}>Was the consultant helpful? </Text>
